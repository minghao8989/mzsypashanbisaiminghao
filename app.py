...
def display_athlete_welcome_page(config):
    ...
    if st.session_state.show_manual_scan_info:
        st.warning("📱 请使用手机扫码应用扫描管理员提供的二维码。")
        if st.button("关闭提示"): st.session_state.show_manual_scan_info = False; st.rerun()
...
def display_admin_data_management(config):
    ...
    with st.form("sys_config"):
        ...
        new_url = st.text_input("基本URL (例如 https://xxxx.streamlit.app)", config['QR_CODE_BASE_URL'])
...
def main_app():
    ...
    page = st.sidebar.radio("功能导航", pages, key='nav_radio', index=pages.index(st.session_state.page_selection))
    ...
    if page == "选手登记":
        ...
        with st.form("reg_form", clear_on_submit=True):
            ...
            if st.form_submit_button("提交报名"):
                df = load_athletes_data()
                if phone in df['phone'].values: st.error("该手机已注册")
                else:
                    new_id = 1001 if df.empty else int(pd.to_numeric(df['athlete_id']).max()) + 1
                    new_df = pd.concat([df, pd.DataFrame([{'athlete_id':str(new_id),'department':dept,'name':name,'phone':phone,'username':name,'password':phone}])])
                    new_df.to_csv(ATHLETES_FILE, index=False, encoding='utf-8-sig')
                    st.success(f"成功! 编号 {new_id}"); time.sleep(1); st.rerun()
        ...
        if not df_calc.empty:
            ...
            st.dataframe(df_final[['排名', 'name', 'department', '用时']], hide_index=True)
        else: st.warning("暂无记录")
    elif page == "数据管理": display_admin_data_management(config)

if __name__ == '__main__':
    st.set_page_config(page_title="赛事系统", page_icon="🏃", layout="wide")
    main_app()
